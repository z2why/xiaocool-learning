		...
		methods: {
			...
			buy() {
				...
				for (var j = 0; j < this.selectedList.length; j++) {
					this.shop.couId = this.selectedList[j]
					this.shop.stuId = this.user.stuId
					this.shop.uid = this.user.id
					console.log(this.selectedList[j])
				...
				// 	uid:"1"
				// }
				// this.thing.push(copy)
				this.$api('tijiao', this.thing).then(res => {
					console.log(res, 'resres')
					if (res.code == 200) {
                      this.orderId = res.data.records.id
					  uni.navigateTo({
